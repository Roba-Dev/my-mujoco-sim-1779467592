...
export function setupGUI(parentContext) {
  ...
  // Add scene selection dropdown — built dynamically from loaded scenes.
  ...
  if (parentContext.sceneFiles && parentContext.sceneFiles.length > 0) {
    ...
    // Fallback: use current initialScene
    let f = parentContext.params.scene;
    let label = f.replace('.xml', '').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    sceneOptions[label] = f;
  }
  parentContext.gui.add(parentContext.params, 'scene', sceneOptions).name('Example Scene').onChange(reload);
  ...
  // ========== CONTROLS FOLDER (expanded by default) ==========
  let controlsFolder = parentContext.gui.addFolder("Controls");

  // Pause simulation checkbox
  const pauseSimulation = controlsFolder.add(parentContext.params, 'paused').name('Pause Simulation');
  ...
  controlsFolder.add({reset: () => { resetSimulation(); }}, 'reset').name('Reset');
  document.addEventListener('keydown', (event) => {
  ...
  controlsFolder.add({resetCamera: resetCamera}, 'resetCamera').name('Reset Camera');
  document.addEventListener('keydown', (event) => {
  ...
  // ========== RELOAD FOLDER (expanded by default) ==========
  let reloadFolder = parentContext.gui.addFolder("Reload");

  // Reload model button
  reloadFolder.add({reload: () => { reload(); }}, 'reload').name('Reload XML');
  document.addEventListener('keydown', (event) => {
    if (event.ctrlKey && event.code === 'KeyL') { reload();  event.preventDefault(); }});
  actionInnerHTML += 'Reload XML<br>';
  keyInnerHTML += 'Ctrl L<br>';

  // ========== SIMULATION FOLDER (expanded by default) ==========
  ...
  // ========== ACTUATORS FOLDER (collapsed by default) ==========
